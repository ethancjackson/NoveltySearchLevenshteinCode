...
exp_name = 'HybridNovelty-LastLayer-NoDump-Test'
...
def evolve_solution(game_name, action_space_size, pop_size=1000+1, archive_p=0.01, archive_dump=None, n_generations=1000,
    ...
    for gen in range(starting_generation, n_generations):
        ...
        pop_trunc = sorted(pop_trunc, key=lambda x: x[1])[-T // 2 + 1:]
...
evolve_solution(game_name, action_space_size, pop_size=1000+1, archive_p=0.01, archive_dump=None,
                training_episodes=[0],
                training_frames=10000, validation_frames=20000, validation_episodes=list(range(1,31)))